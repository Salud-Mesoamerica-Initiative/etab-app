...
class Location(Auditable, Noun):
    ...
    def change_dimension(self, new_dimension):
        self.dimension = new_dimension
        self.save(update_fields=['dimension'])
        self.update_dimension_tags(new_dimension)
    ...
    def get_scores(self):
        return Score.objects.filter(location=self)
...
def update_cached(self, force_check_all):
    ...
    scores = Score.objects.filter(location=self, datetime__gte=year_ago).order_by('datetime')
    invalidated_keys = []
